...
(function () {
  ...
  /* =============================== ESTILOS =============================== */
  /* Cores via var() do site (style.css) com fallback, para casar com a identidade. */
  function injectStyles() {
    ...
    var css =
      ...
      '.hs-cc-banner{position:fixed;left:50%;bottom:20px;transform:translateX(-50%);' +
        'width:calc(100% - 40px);max-width:1060px;background:var(--navy-deep,#012a60);color:#e8edf3;' +
        'border:1px solid rgba(255,255,255,0.12);border-radius:var(--radius-lg,14px);' +
        'box-shadow:0 18px 48px rgba(1,42,96,0.45);z-index:2147483000;padding:22px 26px;' +
      ...
      '.hs-cc-banner-text{flex:1;font-size:13.5px;line-height:1.6;color:#c3cedd;}' +
      ...
      '.hs-cc-banner-text a{color:var(--sky,#4da3f5);text-decoration:underline;}' +
      '.hs-cc-banner-text a:hover{color:#fff;}' +
      ...
      ".hs-cc-btn{font-family:'IBM Plex Sans',system-ui,sans-serif;font-size:13.5px;" +
        'font-weight:600;padding:11px 20px;border-radius:var(--radius-pill,999px);cursor:pointer;' +
        'border:1px solid transparent;transition:all .18s;white-space:nowrap;}' +
      '.hs-cc-btn-primary{background:var(--primary,#017ae2);color:#fff;}' +
      '.hs-cc-btn-primary:hover{background:var(--primary-dark,#0162bc);}' +
      '.hs-cc-btn-ghost{background:transparent;color:#e8edf3;border-color:rgba(255,255,255,0.30);}' +
      '.hs-cc-btn-ghost:hover{background:rgba(255,255,255,0.08);border-color:rgba(255,255,255,0.6);}' +
      '.hs-cc-btn-ghost-dark{background:transparent;color:var(--navy,#013580);border-color:rgba(1,42,96,0.22);}' +
      '.hs-cc-btn-ghost-dark:hover{background:var(--gray-50,#f5f7fa);border-color:rgba(1,42,96,0.45);}' +
      '.hs-cc-overlay{position:fixed;inset:0;background:transparent;z-index:2147483100;' +
        'display:flex;align-items:center;justify-content:center;padding:24px;}' +
      ".hs-cc-modal{background:#fff;border-radius:var(--radius-lg,14px);width:100%;max-width:560px;" +
        'max-height:calc(100vh - 48px);overflow-y:auto;' +
        "font-family:'IBM Plex Sans',system-ui,sans-serif;" +
        'box-shadow:0 24px 64px rgba(1,42,96,0.35);animation:hs-cc-pop .25s ease;}' +
      ...
      '.hs-cc-modal-head{display:flex;align-items:center;justify-content:space-between;' +
        'padding:24px 26px;border-bottom:1px solid rgba(1,42,96,0.10);}' +
      ".hs-cc-modal-head h2{margin:0;font-family:'Archivo',sans-serif;font-size:20px;" +
        'font-weight:700;color:var(--navy-deep,#012a60);letter-spacing:-0.015em;}' +
      '.hs-cc-modal-close{background:none;border:none;cursor:pointer;font-size:26px;' +
        'line-height:1;color:#8a93a0;padding:0 4px;}' +
      '.hs-cc-modal-close:hover{color:var(--navy-deep,#012a60);}' +
      '.hs-cc-modal-body{padding:22px 26px;}' +
      '.hs-cc-modal-intro{margin:0 0 18px;font-size:14px;line-height:1.6;color:var(--gray-700,#3a3f45);}' +
      '.hs-cc-modal-intro a{color:var(--primary,#017ae2);font-weight:500;}' +
      '.hs-cc-cat{padding:16px 0;border-top:1px solid rgba(1,42,96,0.10);}' +
      ...
      ".hs-cc-cat-name{font-family:'Archivo',sans-serif;font-weight:600;font-size:15px;color:var(--navy-deep,#012a60);}" +
      '.hs-cc-cat-desc{margin:6px 0 0;font-size:13px;line-height:1.55;color:var(--gray-700,#3a3f45);}' +
      ...
      '.hs-cc-track{position:absolute;inset:0;background:#ccd4df;border-radius:50px;' +
        'cursor:pointer;transition:background .2s;}' +
      ".hs-cc-track::before{content:'';position:absolute;width:18px;height:18px;left:3px;top:3px;" +
        'background:#fff;border-radius:50%;transition:transform .2s;box-shadow:0 1px 3px rgba(1,42,96,0.4);}' +
      '.hs-cc-switch input:checked + .hs-cc-track{background:var(--primary,#017ae2);}' +
      ...
      '.hs-cc-switch input:focus-visible + .hs-cc-track{box-shadow:0 0 0 3px rgba(1,122,226,0.30);}' +
      '.hs-cc-modal-foot{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;' +
        'padding:20px 26px;border-top:1px solid rgba(1,42,96,0.10);}' +
      '.hs-cc-prefs-link{background:none;border:none;padding:0;margin:0;' +
        "font-family:'IBM Plex Sans',system-ui,sans-serif;font-size:12px;color:#fff;cursor:pointer;" +
        'transition:color .2s;}' +
      '.hs-cc-prefs-link:hover{color:var(--sky,#4da3f5);}' +
